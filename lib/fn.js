...
@param BLcond
@param cur_CanCond
@param dailyStep - daily or monthly?
*/
module.exports.Transp = function(date_nrel, date_daylight, cur_VPD, BLcond, cur_CanCond, dailyStep){
  ...
  var Transp = ( (e20 * netRad + defTerm ) / div ) * date_daylight * 3600 / lambda;

  if( dailyStep ) {
    return Transp;
  }

  return Transp * constant('days_per_month');
};
...
@param molPAR_MJ
*/
module.exports.PAR = function(date_rad, molPAR_MJ, dailyStep ) {
  if( typeof molPAR_MJ === null || typeof molPAR_MJ === 'undefined' ) {
    molPAR_MJ = constant('molPAR_MJ');
  }

  var PAR = date_rad * molPAR_MJ;
  if( dailyStep ) {
    return PAR;
  }

  return PAR * constant('days_per_month');
};
