...
module.exports.Transp = function(date_nrel, date_daylight, cur_VPD, BLcond, cur_CanCond, dailyStep){
  ...
  var result = ( (e20 * netRad + defTerm ) / div ) * date_daylight * 3600 / lambda;

  if( dailyStep ) {
    return result;
  }

  return result * constant('days_per_month');
};
...
module.exports.PAR = function(date_rad, molPAR_MJ, dailyStep ) {
  if( molPAR_MJ === null || typeof molPAR_MJ === 'undefined' ) {
    molPAR_MJ = constant('molPAR_MJ');
  }

  var result = date_rad * molPAR_MJ;
  if( dailyStep ) {
    return result;
  }
