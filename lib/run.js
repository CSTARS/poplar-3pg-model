...
function runSetup(setup){
    ...
    var keysInOrder = [];
    for ( var key in dataModel.plantation_state.value ) {
    ...
    for (var step = 1; step < setup.lengthOfGrowth; step++) {
      ...
      if( this.manage.coppice && this.onHarvest ) {
        var data = {};

        for (var i = 0; i < keysInOrder.length; i++) {
          data[keysInOrder[i]] = nextMonthResults[keysInOrder[i]];
        }
...
function getWeather(model, setup) {
  if( model.customWeather && model.customWeather[setup.currentDate.getFullYear()+'-'+m] ) {
    return model.customWeather[setup.currentDate.getFullYear()+'-'+m];
  }
